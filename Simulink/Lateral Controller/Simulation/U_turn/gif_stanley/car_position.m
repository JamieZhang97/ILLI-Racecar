...
for i=1:135
    ...
    xfl = out.x_track(i) - Wd/2*sin(out.theta_track(i)) + (Lf - Wb)*cos(out.theta_track(i));
    yfl = out.y_track(i) + Wd/2*cos(out.theta_track(i)) + (Lf - Wb)*sin(out.theta_track(i));
    
    xfr = out.x_track(i) + Wd/2*sin(out.theta_track(i)) + (Lf - Wb)*cos(out.theta_track(i));
    yfr = out.y_track(i) - Wd/2*cos(out.theta_track(i)) + (Lf - Wb)*sin(out.theta_track(i));
    
    xrl = out.x_track(i) - Wd/2*sin(out.theta_track(i)) - (Lr + Wb)*cos(out.theta_track(i));
    yrl = out.y_track(i) + Wd/2*cos(out.theta_track(i)) - (Lr + Wb)*sin(out.theta_track(i));
    
    xrr = out.x_track(i) + Wd/2*sin(out.theta_track(i)) - (Lr + Wb)*cos(out.theta_track(i));
    yrr = out.y_track(i) - Wd/2*cos(out.theta_track(i)) - (Lr + Wb)*sin(out.theta_track(i));
    ...
    %find wheel center
    xwflo = out.x_track(i) - (Wd - Ww)/2*sin(out.theta_track(i));
    ywflo = out.y_track(i) + (Wd - Ww)/2*cos(out.theta_track(i));
    
    xwfro = out.x_track(i) + (Wd - Ww)/2*sin(out.theta_track(i));
    ywfro = out.y_track(i) - (Wd - Ww)/2*cos(out.theta_track(i));
    
    xwrlo = out.x_track(i) - (Wd - Ww)/2*sin(out.theta_track(i)) - Wb*cos(out.theta_track(i));
    ywrlo = out.y_track(i) + (Wd - Ww)/2*cos(out.theta_track(i)) - Wb*sin(out.theta_track(i));
    
    xwrro = out.x_track(i) + (Wd - Ww)/2*sin(out.theta_track(i)) - Wb*cos(out.theta_track(i));
    ywrro = out.y_track(i) - (Wd - Ww)/2*cos(out.theta_track(i)) - Wb*sin(out.theta_track(i));
