for i=1:261
    ...
    plot(out.x_ref(i+10), out.y_ref(i+10),'*b','linewidth',4)
    ...
    vector = plot([out.x_track(i) out.x_track(i)+0.6*cos(out.theta_track(i))], [out.y_track(i) out.y_track(i)+0.6*sin(out.theta_track(i))],'red', 'linewidth',1);
    
    x_tri = [out.x_track(i)+0.6*cos(out.theta_track(i)), out.x_track(i) + 0.6*cos(out.theta_track(i)) - 0.15*cos(out.theta_track(i)+0.2), out.x_track(i) + 0.6*cos(out.theta_track(i)) - 0.15*cos(out.theta_track(i)-0.2), out.x_track(i)+0.6*cos(out.theta_track(i))];
    y_tri = [out.y_track(i)+0.6*sin(out.theta_track(i)), out.y_track(i) + 0.6*sin(out.theta_track(i)) - 0.15*sin(out.theta_track(i)+0.2), out.y_track(i) + 0.6*sin(out.theta_track(i)) - 0.15*sin(out.theta_track(i)-0.2), out.y_track(i)+0.6*sin(out.theta_track(i))];
